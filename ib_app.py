...
def xf(word, fromLang, toLang):
    word = quote(word)
    url = "http://www.xfyun.cn/herapi/product/translate?fromLang="+fromLang+"&toLang="+toLang+"&originText="+word
    res = nuggets_worm(url=url)
    return res


def SetupLogger():
    if not os.path.exists("IB_trader_log"):
        os.makedirs("IB_trader_log")
    ...
    logging.basicConfig(filename=time.strftime("IB_trader_log/pyibapi.%y%m%d_%H%M%S.log"),
                        filemode="w",
                        level=logging.INFO,
                        format=recfmt, datefmt=timefmt)
...
class IBApp(TestWrapper, TestClient):
    ...
    def start(self):
        ...
        if self.globalCancelOnly:
            ...
            print("global cancel is successed")
        else:
            print("no global cancel")
            # self.reqMatchingSymbols(int(time.time()), 'TSLA')
            # self.reqContractDetails(int(time.time()), test_contract)
    ...
    def orderStatus(self, orderId: OrderId, status: str, filled: float,
        ...
        self.order_list.append([{"client_id": clientId, "order_id": orderId, "status": status, "filled": filled,
                                 "remaining": remaining, "avg_price": avgFillPrice, "last_price": lastFillPrice}])
    ...
    def accountSummary(self, reqId: int, account: str, tag: str, value: str,
        ...
        self.account_list.append({"account": account, "tag": tag, "value": value, "currency": currency})
        self.account_flag = False
    # ! [accountsummary]
    ...
    def positionMulti(self, reqId: int, account: str, modelCode: str,
        ...
        position = {
            ...
            "sec_type": contract.secIdType,
            "symbol": contract.symbol,
            "pri_exc": contract.primaryExchange,
    ...
    def historicalData(self, reqId:int, bar: BarData):
        if len(bar.date) == 8:
            time_array = time.strptime(bar.date, "%Y%m%d")
        else:
            time_array = time.strptime(bar.date, "%Y%m%d %H:%M:%S")
        time_stamp = time.mktime(time_array)
        df = [float(time_stamp), bar.open, bar.high, bar.low, bar.close, bar.volume, bar.barCount, bar.average]
        self.bar_list.append(df)
    ...
    def symbolSamples(self, reqId: int,
        ...
        for contractDescription in contractDescriptions:
            ...
            for derivSecType in contractDescription.derivativeSecTypes:
                ...
                derivSecTypes += "/"
            contract_dict['conId'] = contractDescription.contract.conId
            contract_dict['symbol'] = contractDescription.contract.symbol
            contract_dict['secType'] = contractDescription.contract.secType
            contract_dict['primaryExchange'] = contractDescription.contract.primaryExchange
            contract_dict['currency'] = contractDescription.contract.currency
            contract_dict['derivSecTypes'] = derivSecTypes
            self.contracts_list.append(contract_dict)
        # print("start获得结果时间：", int(time.time()))
    # ! [symbolSamples]
    ...
    def fundamentalData(self, reqId: TickerId, data: str):
        ...
        if self.report_type == "ReportsFinStatements":
            # 使用etree模块解析xml格式的财报
            tree = ET.fromstring(data)
            # 对应的英文
            # en_dict = {}
            # for element in tree.iter(tag="mapItem"):
            #     en = element.get("coaItem")
            #     en_dict[element.tag] = en
            translate_dict = {'SREV': '主营业务收入', 'SORE': '其他业务收入，总额', 'RTLR': '总收入', 'SCOR': '总成本',
                              'SGRP': '主营业务利润', 'SSGA': '销售及管理费用', 'ERAD': '研发成本',
                              'SDPR': '折旧/摊销', 'SINN': '总营业额', 'SUIE': '营业外收入',
                              'SOOE': '其他经营费用总额', 'ETOE': '总经营费用', 'SOPI': '营业收入',
                              'SNIN': '扣除税（含税）净非营业额，总额', 'NGLA': '出售资产的收益（损失）', 'SONT': '其他，净值',
                              'EIBT': '税前净收入', 'TTAX': '所得税准备金', 'TIAT': '税后净收入', 'CMIN': '少数人利益',
                              'CEIA': '分支机构的股权', 'CGAP': '美国会计准则调整项目',
                              'NIBX': '在特殊项目前的净收入', 'STXI': '特殊项目总额',
                              'NINC': '净收入', 'SANI': '净收入的总调整额', 'CIAC': '不含额外特殊项目的可分配收入',
                              'XNIC': '含额外特殊项目的可分配收入', 'SDAJ': '摊薄调整额', 'SDNI': '摊薄净收入',
                              'SDWS': '摊薄加权平均股数', 'SDBF': '扣除额外项目的每股收益',
                              'DDPS1': '普通股首次发行', 'VDES': '稀释标准化EPS',
                              'ACSH': '现金流', 'ACAE': '现金及等价物', 'ASTI': '现金和短期投资',
                              'SCSI': '现金和短期投资', 'AACR': '应收账款 -贸易，净值',
                              'ATRC': '应收账款总额，净值', 'AITL': '总库存', 'APPY': '预付费用',
                              'SOCA': '其他流动资产总额', 'ATCA': '流动资产总额', 'APTC': '物业/厂房/设备，合计 - 总额',
                              'ADEP': '累计折旧总额', 'APPN': '物业/厂房/设备，总计 - 净值', 'AGWI': '商誉', 'AINT': '无形资产',
                              'SINV': '长期投资', 'ALTR': '应收票据-长期', 'SOLA': '其他长期资产总额',
                              'ATOT': '总资产', 'LAPB': '应付账款', 'LPBA': '应付账款',
                              'LAEX': '应付/计提', 'LSTD': '应付票据/短期债务',
                              'LCLD': '现有长期债务/资本租赁', 'SOCL': '其他流动负债，总计',
                              'LTCL': '流动负债总额', 'LLTD': '长期债务', 'LCLO': '资本性租赁下的长期债务',
                              'LTTD': '总长期债务', 'STLD': '总债务', 'SBDT': '延期所得税',
                              'LMIN': '少数人利息', 'SLTL': '其他债务总额', 'LTLL': '总负债',
                              'SRPR': '可赎回的优先股总额', 'SPRS': '优先股 - 不可赎回，净值',
                              'SCMS': '普通股，总额', 'QPIC': '额外支付的资本', 'QRED': '留存收益（累计赤字）',
                              'QTSC': '库存股 - 通用', 'QEDG': 'ESOP债务担保', 'QUGL': '未实现收益（亏损）',
                              'SOTE': '其他权益总额', 'QTLE': '总股本', 'QTEL': '负债总额和股东权益',
                              'QTCO': '发行在外普通股总额', 'QTPO': '发行在外的优先股总额', 'STBP': '每股有形帐面价值，普通股',
                              'ONET': '净收入/起点', 'SDED': '折旧/损耗', 'SAMT': '摊销',
                              'OBDT': '递延税', 'SNCI': '非现金物品', 'SOCF': '营运资金的变化',
                              'OTLO': '经营活动中的现金', 'SCEX': '资本支出', 'SICF': '其他投资现金流量项目',
                              'ITLI': '现金支出', 'SFCF': '融资现金流项目', 'FPSS': '股票发行（退休），净额',
                              'FPRD': '股票发行（退休），净额', 'FTLF': '来自融资活动的现金',
                              'SFEE': '外汇影响', 'SNCC': '现金净变化', 'SCIP': '已付现金利息', 'SCTP': '已付现金税'}
            for element in tree.iter(tag="mapItem"):
                en = element.get("coaItem")
                cn = ""
                res_flag = 0
                ...
                while res_flag <= 4:
                    try:
                        res = json.loads(xf(element.text, "en", "cn"))
                        if res['desc'] == "成功":
                            cn = res['data']
                            translate_dict[en] = cn
                            break
                        else:
                            res_flag += 1
                    except:
            ...
            # 完成翻译
            statement_list = []
            ticker = tree.find(".//IssueID[@Type='Ticker']")
            statements = tree.findall(".//Statement")
            for statement in statements:
                statement_dict = {'ticker': ticker.text, 'type': statement.attrib['Type']}
                # 获取FP头
                for child in statement:
                    if child.tag == "lineItem":
                        statement_dict[translate_dict[child.attrib['coaCode']]] = child.text
                    elif child.tag == "FPHeader":
                        period_num = "M"
                        for grandson in child:
                            if grandson.tag == "StatementDate":
                                time_array = time.strptime(grandson.text, "%Y-%m-%d")
                                statement_dict['statement_date'] = time.mktime(time_array)
                                statement_dict['报表日期'] = grandson.text
                            elif grandson.tag == "Source":
                                statement_dict['source_type'] = grandson.text
                                statement_dict['发布日期'] = grandson.attrib['Date']
                                time_array = time.strptime(statement_dict['发布日期'], "%Y-%m-%d")
                                statement_dict['source_date'] = time.mktime(time_array)
                            elif grandson.tag == "UpdateType":
                                statement_dict['update_type'] = grandson.attrib['Code']
                                # statement_dict['update_status'] = grandson.text
                            elif grandson.tag == "PeriodLength":
                                period_num = grandson.text
                            elif grandson.tag == "PeriodType":
                                statement_dict['period'] = period_num+grandson.attrib['Code']
                            else:
                                pass

                statement_list.append(statement_dict)

            statement_sorted = sorted(statement_list, key=lambda x: x['source_date'])
            for i in statement_sorted:
                if i['type'] == "BAL":
                    self.bal_list.append(i)
                elif i['type'] == "INC":
                    self.inc_list.append(i)
                elif i['type'] == "CAS":
                    self.cas_list.append(i)
                else:
                    pass
            # statement_list.append(statement_dict)
        # print(statement_list)
        # for statement in statements.itertext():
        #     print(statement)
        else:
            print(data)
    # ! [fundamentaldata]
